class Pedestrian {
    ...
    getRoute(startX, startY) {
      ...
      while (!nextNode) {
        let rX = ~~(Math.random() * window.map.length);
        let rY = ~~(Math.random() * window.map[0].length);
        if (window.graph.grid[rX][rY].weight == 1) {
          nextNode = {
            x: rX,
            y: rY
          }
      ...
      this.route = astar.search(window.graph, window.graph.grid[startX][startY], window.graph.grid[nextNode.x][nextNode.y]);
      this.step = 0;
